...
var getCardById = (id, callback) =>
{
    console.log("Calling getCard with id " + id);
    var getCard = fs.readFileSync(path.join(__dirname + '/sql/get_card_by_id.sql')).toString();

    db.any(prepSQL(getCard), [id])
    .then((response) =>
    {
        if (response != undefined && response.length != 0)
        {
    ...
    .catch((error) =>
    {
        console.log('[Alert] Error retrieving card with ID ' + id);
        console.log(error);
        return callback("");
    });
}
