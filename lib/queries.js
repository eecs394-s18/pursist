...
/*
    Exporting from Postgres to CSV file local
*/

var exportCSV = (callback) =>
{
    console.log("EXPORT CSV CALLED")
    var exportCSV = fs.readFileSync(path.join(__dirname + '/sql/export_csv.sql')).toString();
    db.any(prepSQL(exportCSV), [])
